...
chart2 <- function(pic.path,
     ...
     pkgs <- c('tiff', 'terra', 'jpeg', 'sm', 'spatstat', 'RImageJROI', 'stringr', 'librarian', 'varhandle', 'autothresholdr')
     for(i in pkgs){
    ...
    if(interactive == T){
          if(xriteclassic.chart == T){
                ...
                sp.chart <- terra::rast(chart.coords[, c("x", "y")])
          }else{
    ...
    sp.chart <- terra::rast(x = chart.coords)
