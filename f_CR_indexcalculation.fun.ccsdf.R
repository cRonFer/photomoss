...
index.calc.fun <- function(raster.mat, # raster.mat  = calibration_results[[1]]
                           raster.band, # raster.band = calibration_results[[2]]
    ...
    if(length(grep(paste(c("NORR", "NORG", "NORB", "EXR", "EXG", "EXB", "EXGR", "CIVE","VEG"), collapse = "|"), unique(index.))) > 0){
      ...
      rst <- raster(rgb_norm[[3]])# set reference raster
    }
    ...
    if(any(unique(grepl("EXR", index.))) == TRUE){
      EXR <- setValues(rst, 1.4 * getValues(rgb_norm[[1]]) - getValues(rgb_norm[[2]]))
      }else{
      EXR <- NULL
      }
    # Excess green (ExG)
    if(any(unique(grepl("^EXG$", index.))) == TRUE){
      EXG <- setValues(rst,2 * getValues(rgb_norm[[2]]) -
    ...
    if(any(unique(grepl("EXB", index.))) == TRUE){
      EXB <- setValues(rst, 1.4 * getValues(rgb_norm[[3]]) - getValues(rgb_norm[[2]]))
    }else{
      EXB <- NULL
    }
    ...
    if(any(unique(grepl("^CIVE$", index.))) == TRUE){
      CIVE <- setValues(rst, 0.441 * getValues(rgb_norm[[1]]) -
    ...
    if(any(unique(grepl("VEG", index.))) == TRUE){
      VEG <- setValues(rst, getValues(rgb_norm[[2]]) /
